...
async function fetchCountryInfo(nameCountry) {
    try {
        const response = await _axiosDefault.default.get(`https://restcountries.com/v2/name/${nameCountry}`);
        messageText.textContent = " ";
...
function createListItem(country) {
    ...
    if (country.currencies.length > 1) countryCapText.textContent += ` and ${country.currencies[1].name}'s`;
...
},{}]},["5bH3W","bNKaB"], "bNKaB", "parcelRequire5e27")
